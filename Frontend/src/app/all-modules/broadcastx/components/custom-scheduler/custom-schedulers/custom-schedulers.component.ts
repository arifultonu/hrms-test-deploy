...
declare const $: any;
@Component({
  selector: 'app-custom-schedulers',
  templateUrl: './custom-schedulers.component.html',
  styleUrls: ['./custom-schedulers.component.css']
})
export class CustomSchedulersComponent implements OnInit {
 ...
 startJob(event,id:number,jobTitle:String) {
   ...
   var urlLink;
    ...
    if(jobTitle=="BACK_DAY_ATTN_PROC")
    {
      urlLink="/systemJob/backDateAttnProc/";
    }
    else
    {
 ...
 private getUserQueryParams(page: number, pageSize: number): any {
   ...
   params[`jobType`] = "CUSTOM_SCHEDULER";
