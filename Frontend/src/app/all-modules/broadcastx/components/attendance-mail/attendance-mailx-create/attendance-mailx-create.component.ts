...
@Component({
  selector: 'app-attendance-mailx-create',
  templateUrl: './attendance-mailx-create.component.html',
  styleUrls: ['./attendance-mailx-create.component.css']
})
export class AttendanceMailxCreateComponent implements OnInit {
  ...
  formSubmit()
  {
    ...
    this.broadcastxService.sendPostRequest(apiURL, formData).subscribe(
      (response: any) => {
        ...
        this.router.navigate(["/broadcastx/attendance-mail"], {relativeTo: this.route});
      },
      (error) => {
        console.log(error);
