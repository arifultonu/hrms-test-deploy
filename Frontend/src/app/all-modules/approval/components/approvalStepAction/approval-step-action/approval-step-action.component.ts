...
declare const $: any;
@Component({
  selector: 'app-approval-step-action',
  templateUrl: './approval-step-action.component.html',
  styleUrls: ['./approval-step-action.component.css']
})
export class ApprovalStepActionComponent implements OnInit {
  ...
  getListData()
  {
    let apiURL = this.baseUrl + "/approvalStepAction/getAll";
  ...
  {
    let apiURL = this.baseUrl + "/approvalStepAction/delete/" + dataId;
    console.log(apiURL);
  ...
  private getUserQueryParams(page: number, pageSize: number): any {
    ...
    if(this.approvalProcess){
      params[`approvalProcess`] = this.approvalProcess;
    }
  ...
  //search
   ...
   searchEntity(paramName, entityType) {
    ...
    if(entityType=='approvalProcess'){
    this.approvalProcess = paramName;
