...
export class ManagementListComponent implements OnInit {
  ...
  searchByEmpCode(val) {
    this.srcEmpCode=val;
    //this.srcEmpCode = this.myFromGroup.get('srcEmpCode').value;
  }
  ...
  _getListData(){
    let apiURL = this.baseUrl + "/sim/getSimManagement";
  ...
  private _getUserQueryParams(page: number, pageSize: number): any {
    ...
    if(this.srcEmpCode){
      params[`allotNumber`] = this.srcEmpCode;
    }
    if(this.srcStatus){
  ...
  public deleteEnityData( dataId ){
    ...
    let apiURL = this.baseUrl + "/sim/deleteSimManagement/" ;
    console.log(apiURL);
